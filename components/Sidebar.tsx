        ...
        {filteredTasks.map((task) => (
          <button
            ...
            className={`w-full text-left p-3 rounded-xl flex items-center gap-4 transition-all duration-200 group relative ${
              ...
              ? 'bg-blue-50 dark:bg-blue-900/10 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-900/20 shadow-sm' 
      ...
      <div className="p-6 border-t border-slate-100 dark:border-slate-900 shrink-0">
        <div className="flex justify-center items-center gap-6 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${theme === 'light' ? 'bg-white shadow-md text-blue-600' : 'text-slate-500 hover:text-slate-300'}`}
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707m12.728 0A9 9 0 115.552 13.292c.337-.363.486-.83.462-1.312z" /></svg>
          </button>
          <button 
            onClick={() => theme === 'light' && toggleTheme()}
            className={`p-2 rounded-lg transition-all ${theme === 'dark' ? 'bg-slate-800 shadow-md text-yellow-400' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
          </button>
        </div>
      </div>
    </aside>
